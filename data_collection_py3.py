...
'''
This version of the data collection code has been adapted from the official CosmicWatch v2 import_data.py script.
It can be run with python3 and includes further options for saving data files to your computer.
Your computer driven data collection now will save to a file with the (1) detector name (2) date (3) total runtime, and includes the ability to set a run time, after which the file is closed and data collection stops.

last updated: June 25, 2026 -- Samantha Muller
'''
...
if mode == 1:
        ...
        while True:
                run_minutes = input("Enter run length in minutes: ").strip()

                try:
                        run_minutes_float = float(run_minutes)
                        break
                except ValueError:
        ...
        for i in range(nDetectors):
                ...
                if det_name not in valid_detector_names:
                        print('--- Error ---')
                        print('Detector name not recognized: ' + det_name)
                        print('Expected one of:')
                        print(valid_detector_names)
                        print('Exiting ...')
                        exit()
        ...
        if nDetectors > 1:
                ...
                string_of_names = string_of_names.rstrip(', ')
        else:
                string_of_names += detector_name_list[0]

        date_str = datetime.now().strftime("%Y-%m-%d")

        detector_file_name = string_of_names.replace(", ", "_")
        detector_file_name = detector_file_name.replace(" ", "_")
        detector_file_name = detector_file_name.replace("/", "_")

        fname = os.path.join(
                data_dir,
                detector_file_name + "_" + date_str + "_" + run_minutes + "min.txt"
        )
...
#        file.write(header5.decode())
        ...
        print("\nTaking data ...")
        print("Data collection will stop after " + run_minutes + " minutes.")
        print("Press ctl+c to terminate process early.")

        start_time = time.time()
        run_seconds = run_minutes_float * 60
...
if mode == 4:
    ...
    #bg.daemon = True
    #bg.start()
    _thread.start_new_thread(RUN,(bg,)) 
